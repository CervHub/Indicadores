...
export function GenericInpsCat({ title, pageType, companies, entities, categoriesData }: ChartCatProps) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await axios.post(route('year.metrics.inspeccion.detalle', { type: 'cat' }), formData);
                        ...
                        <table className="min-w-full divide-y divide-gray-200 rounded-full dark:divide-gray-700">
                            ...
                            <tbody className="divide-y divide-gray-200 rounded-b-sm bg-white dark:divide-gray-700 dark:bg-gray-900">
                                {sortedChartData.map((data, index) => (
                                    <tr key={index}>
                                        <td className="max-w-[300px] truncate px-2 py-3 text-sm font-medium whitespace-nowrap text-gray-900 dark:text-gray-100">
                                            {data.report}
                                        </td>
