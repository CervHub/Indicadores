...
export default function InspectionVehicle({
    ...
    const { data, setData, reset } = useForm({
        ...
        causas: causas.map((causa) => ({ id: causa.id, state: '', observation: '' })), // Agregar causas al formulario
    });
    ...
    const handleCausaStateChange = (id: string, state: string, observation: string) => {
        const updatedCausaStates = causaStates.map((causa) =>
            causa.id === id
                ? {
                      ...causa,
                      state: state || causa.state, // Preserve the current state if no new state is provided
                      observation,
                  }
                : causa,
        );
        setCausaStates(updatedCausaStates);
        setData('causas', updatedCausaStates); // Update causas in useForm data
    };
        ...
        fetch(route('web.v1.searchVehicles', { plate: data.plate }))
            ...
            .then((response) => {
                ...
                if (vehicleData) {
                    ...
                    setData('engineNumber', vehicleData.engine_number || '');
        ...
        fetch(route('web.v1.searchUsers', { licenseNumber: data.licenseNumber }))
            ...
            .then((response) => {
                ...
                if (userData) {
                    ...
                    setData('company', userData.empresa || '');
